// src/pages/pole/Plannings.jsx — VERSION FINALE CORRIGÉE
...
// semestre = "S1","S2"... → number pour le filtre
const semestreOptions = ["S1","S2","S3","S4","S5","S6"];

// eg_et: "EG" = Régionale, "ET" = Locale
const getType = (eg_et, is_regional) => {
  if (eg_et === "EG" || is_regional == 1) return "Régionale";
  return "Locale";
};
...
function ModalCreer({ onClose, onSaved, groupes, formateurs }) {
  const [form, setForm] = useState({
    ...
    semestre: "S2", type: "Régionale", mh_drif: "",
  ...
  // Quand groupe change → charge modules de ce groupe
  ...
  // Quand module change → auto-fill formateur + type + MH
  const handleModuleChange = (moduleId) => {
    set("module_id", moduleId);
    const mod = modules.find(m => String(m.id) === String(moduleId));
    if (mod) {
      // Auto-fill type depuis eg_et
      set("type", getType(mod.eg_et, mod.is_regional));
      // Auto-fill MH depuis mh_drif
      if (mod.mh_drif) set("mh_drif", String(mod.mh_drif));
      // Auto-fill semestre
      if (mod.semestre) set("semestre", mod.semestre);
      // Auto-fill formateur si module en a déjà un
      if (mod.formateur_id) set("formateur_id", String(mod.formateur_id));
    }
  };
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ padding: 20 }}>
            ...
            <select style={inp} value={form.groupe_id} required
              ...
              {groupes.map(g => (
                <option key={g.id} value={g.id}>
                  {g.nom ?? g.code ?? g.intitule ?? `Groupe ${g.id}`}
                  {g.filiere_nom ? ` — ${g.filiere_nom}` : ""}
                  {g.annee_formation ? ` (A${g.annee_formation})` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div style={{ ...grp, marginBottom: 14 }}>
            <label style={lbl}>
              Module *
              {!form.groupe_id && <span style={{ color: "#94a3b8", fontWeight: 400, marginLeft: 6 }}>
                — choisir un groupe d'abord
              </span>}
              {loadingMod && <span style={{ color: "#94a3b8", fontWeight: 400, marginLeft: 6 }}>
                — chargement…
              </span>}
              {form.groupe_id && !loadingMod && modules.length === 0 && <span style={{ color: "#ef4444", fontWeight: 400, marginLeft: 6 }}>
                — aucun module pour ce groupe
              </span>}
            </label>
            <select style={{ ...inp, opacity: (!form.groupe_id || loadingMod) ? 0.5 : 1 }}
          ...
          <div style={{ ...grp, marginBottom: 14 }}>
            ...
            <select style={inp} value={form.formateur_id} required
              onChange={e => set("formateur_id", e.target.value)}>
              <option value="">Sélectionner un formateur</option>
              {formateurs.map(f => (
                <option key={f.id} value={f.id}>
                  {f.name ?? f.nom_complet ?? f.nom ?? `Formateur ${f.id}`}
          ...
          {/* Ligne 4 : Semestre + Type + MH */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 14 }}>
            <div style={grp}>
              <label style={lbl}>Semestre *</label>
              <select style={inp} value={form.semestre} required
            ...
            <div style={grp}>
              ...
              <select style={inp} value={form.type} required
                onChange={e => set("type", e.target.value)}>
                <option value="Régionale">Régionale (EG)</option>
                <option value="Locale">Locale (ET)</option>
              </select>
            </div>
            <div style={grp}>
              <label style={lbl}>MH DRIF *</label>
              <input type="number" style={inp} value={form.mh_drif}
                required min="1" placeholder="ex: 90"
                onChange={e => set("mh_drif", e.target.value)} />
            </div>
          </div>

          {/* Ligne 5 : Date + Nb semaines */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
            ...
            <div style={grp}>
              <label style={lbl}>Nb semaines</label>
              <input type="number" style={inp} value={form.nb_semaines}
                min="1" placeholder="ex: 9"
                onChange={e => set("nb_semaines", e.target.value)} />
            </div>
          </div>

          {/* Ligne 6 : Placement EDT */}
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10 }}>
              <div style={grp}>
                ...
                <select style={inp} value={form.jour} onChange={e => set("jour", e.target.value)}>
                  <option value="">—</option>
              ...
              <div style={grp}>
                ...
                <select style={inp} value={form.seance_numero}
              ...
              <div style={grp}>
                ...
                <input type="text" style={inp} value={form.salle}
...
export default function Plannings() {
  ...
  const groupesList = [...new Set(data.map(r => r.groupe).filter(Boolean))];
      ...
      <div className="table-card">
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="table-empty" style={{ padding: "40px 20px" }}>
            <div style={{ marginBottom: 12, color: "#64748b" }}>Aucun planning trouvé.</div>
            <button className="btn btn--primary" onClick={() => setModal(true)}
        ...
        {!loading && !error && filtered.length > 0 && tab === "groupes" && (
          <div className="table-scroll">
            <table className="planning-table">
              <thead>
                <tr>
                  <th>Groupe</th><th>Module</th><th>Type</th>
